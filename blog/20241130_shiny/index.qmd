...
description: "Ich habe jetzt einen Google Cloud Computing Account und hab eine Shiny App drauf laufen"
author: Manuel Reif 
date: 2024-11-30
lang: de
...
<img src="media/main_pic.webp" alt="State Space Model" style="float: right; width: 34%; margin-left: 20px; margin-bottom: 10px;"/>

300$ darf man in den ersten 3 Monaten verbraten wenn man sich bei Google Cloud 
anmeldet. Natürlich nur mit Diensten die Google auf seiner 7. Wolke jedes 
Data Scientisten zur Verfügung stellt. Um ehrlich zu sein, da gibt es viele 
Möglichkeiten sein Geld auszugeben. Manches klingt kompliziert, manches exotisch
...
zur Verfügung hat, könnte es sich schon auszahlen, hin und wieder die Cloud zu
nutzen -- zumindest bin ich mal neugierig.
Ich registriere mich also, schaue ein paar Videos dazu und überfliege ein
paar Tutorials, Anwendungsberichte etc. und komme zu dem Schluss, dass ich 
möglichst schnell mal was ausprobieren will, denn die 3 Monate scheinen
schon in greifbarer Nähe wenn ich die Monate durchzähle. Also versuche ich
mal eine Shiny-App zu deployen. 
...
Allerdings ist die Anzahl der Apps die man gratis hosten 
kann begrenzt und der nächste Schritt ist gleich ein Abo mit 13$ im Monat.
Obwohl meine Apps ziemlich fancy sind, sind die Zugriffszahlen nicht so üppig,
dass ich so mir nichts dir nichts, 13$ mal fix einplanen müsste.
Abeits davon wäre es auch mal interessant was anderes
auszuprobieren.
...
Als ich später angefangen hab R zu nutzen, fühlte ich mich dann und wann doch
...
jedoch halte ich Shiny für hervorragend geeignet statistische Konzepte
...
- Klicken Sie Punkte auf die Abbildung (Achtung nicht zu weit links oder rechts)
...
<img src="media/fremder_computer.jpg" alt="State Space Model" style="float: right; width: 55%; margin-left: 20px; margin-bottom: 10px;"/>

"Never change a running Data Science Setup" ist nicht wirklich ein Sprichwort
und trotzdem eine Prämisse nach der viele Menschen, nennen wir sie "Statistiker", 
handeln die gerade mitten 
in einem Daten Projekt stecken. Ein Update ist ja schön und gut, aber
...
sein um das gut zu finden -- schließlich will man dass sein Code auch noch in
ein paar Jahren läuft, auch wenn man nach Beendigung eines Projekts hofft, nie 
wieder seinen Code sehen zu müssen.
...
das **ganze Drumherum** (ja sogar das Betriebssystem) in einen Container und
...
5. Jetzt kommen die R packages die wir brauchen für unsere Shiny App!
...
Der Klick auf "Container erstellen" beschert uns die erste Auswahl. 
...
1. einen passenden Namen überlegen
...
und Fehlversuchen stellte sich heraus dass der Port **3838** hier angegeben werden
muss und **nicht** 8080 wie der default ist!
...
Ja gut. Dann kann man sich noch mit RAM und CPUs spielen und sehen wohin 
...
ist sie auch hervorragen via Internet erreichbar. 
...
Das wars. Viel Spaß mit der Shiny App!
