...
author: Manuel 
date: 2024-01-22
categories: [quarto, Python, bash, Linux]
...
immer so. Früher, in der guten alten Zeit, habe ich wie jeder vernünftige Mensch
...
Dieser Ablauf soll folgendermaßen geschehen:
...
Bisher hatte ich für solche Aufgaben Makefiles. Damit bin ich nicht so schlecht
...
Ok, unter der Bedingung ich schreib mir so einen Ablauf in Python, dann will ich
folgendes:

1. Multiple `.qmd` Files sollen parallel gerendert etc. werden
...
Das ist vermutlich der einfachste Part. Man muss nur sicherstellen `quarto`
...
2. Was viel cooler ist: in der bash rendern.
...
tool. Für decktape gibt es auch einen [Dockercontainer](https://hub.docker.com/r/astefanutti/decktape) was eine Anwendung
wirklich einfach macht. Man muss sich nicht darüber Gedanken machen, welche
...
Um jetzt eine html Präsentation (revealjs) in pdf Folien zu verwandeln übergebe
ich den folder an den docker container. Und das funktioniert folgendermaßen:
...
1. `docker run` okay, wir wollen irgendwas in docker machen, also soll der container der später genannt wird (`astefanutti/decktape`) laufen
2. `--rm` es wird saubergemacht nachdem docker fertig ist
3. `-t` damit sehen wir was passiert (stdin and stdout werden attached)
4. `-v` wir wollen was mounten und zwar: `ordner/auf/meinem/computer:/container`. Der container rechtnet damit, dass das html file im Ordner "slides" liegt, daher wird der
Ordner vom eigenen Rechner dort reingemountet.
5. `--size='2048x1536'` so funktioniert es bei mir. Andere Einstellungen machen Probleme.
...
Um Handouts zu erstellten, die 4 Folien auf einer Seite darstellen,
...
Wir brauchen nur die dargestellte Zeile um ein Handout zu erhalten.
...
Jetzt haben wir alle einzelnen Schritte beisammen um unsere Präsentation zu erstellen,
...
Die Dateinamen werden von den `quarto` Quarto Dokumenten übernommen und
...
soll von dem Folder auch ein backup gemacht werden (das wird hier jetzt nicht
im Detail dargestellt).
2. Hier hat mir als Noob ChatGPT geholfen. Meine schön programmierte Schleife
wurde zu diesem Konstrukt umformuliert.
3. Wenn 'save' angegeben wird, wird dieser Block getriggert.
...
notwendig). Skripts werden gerne zentral im `~/bin` Ordner gespeichert. Kann man 
machen, muss man aber nicht. Wenn man das will (ich empfehle es zu wollen), 
und hat keinen derartigen 
Ordner, macht an zuerst `mkdir bin` und dann `cp render_praes.py ~/bin` um das 
Skript reinzukopieren.
4. Dann fügt man einen `alias` in die `~/.bashrc` ein. 
Das ist der Moment, in dem man sich entscheiden
muss mit welchem Namen man dieses Skript aufrufen will. 
...
`render_preas`. Boom {{< bi rocket >}}. Es sollte nun zuerst quarto rendern, decktape das
pdf erzeugen und pdfjam anschließend das Handout. Und währenddessen kann
man mal gemütlich einen Kaffee trinken gehen.
